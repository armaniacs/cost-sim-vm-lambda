...
try:
    ...
    pytest.skip(f"Monitoring API module not available: {e}", allow_module_level=True)
...
class TestMetricsEndpoints:
    ...
    def test_get_metrics_with_observability(self, mock_current_app, client):
        ...
        response = client.get(
            "/api/v1/monitoring/metrics?metrics=cpu_usage&metrics="
            "memory_usage&time_range=3600"
        )
        assert response.status_code == 200
...
class TestAlertsEndpoints:
    ...
    def test_alert_search_no_monitoring(self, client):
        ...
        search_data = {"severity": "high", "time_range": 3600}

        response = client.post(
            "/api/v1/monitoring/alerts/search",
            json=search_data,
            content_type="application/json",
        )
        assert response.status_code == 500
...
class TestTracesEndpoints:
    ...
    def test_trace_search_no_monitoring(self, client):
        ...
        search_data = {"operation": "api_call", "min_duration": 100, "time_range": 3600}

        response = client.post(
            "/api/v1/monitoring/traces/search",
            json=search_data,
            content_type="application/json",
        )
        assert response.status_code == 500
