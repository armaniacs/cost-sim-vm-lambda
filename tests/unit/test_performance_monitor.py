...
class TestPerformanceMetrics:
    ...
    def test_record_request(self):
        ...
        metrics.record_request(
            endpoint="/api/test", method="GET", duration=0.5, status_code=200
        )
    ...
    def test_record_request_error(self):
        ...
        metrics.record_request(
            endpoint="/api/error", method="POST", duration=1.0, status_code=500
        )
    ...
    def test_get_average_response_time_specific_endpoint(self):
        ...
        assert metrics.get_average_response_time("GET /api/test") == pytest.approx(
            0.2, rel=1e-9
        )
        assert metrics.get_average_response_time("POST /api/other") == pytest.approx(
            0.5, rel=1e-9
        )
...
class TestCachedWithMonitoringDecorator:
    ...
    def test_cached_with_monitoring_cache_hit(
        self, mock_performance_monitor, mock_get_redis_client
    ):
    ...
    @patch("app.services.performance_monitor.get_redis_client")
    @patch("app.services.performance_monitor.performance_monitor")
    def test_cached_with_monitoring_cache_miss(
        self, mock_performance_monitor, mock_get_redis_client
    ):
    ...
    @patch("app.services.performance_monitor.get_redis_client")
    @patch("app.services.performance_monitor.performance_monitor")
    def test_cached_with_monitoring_redis_error(
        self, mock_performance_monitor, mock_get_redis_client
    ):
