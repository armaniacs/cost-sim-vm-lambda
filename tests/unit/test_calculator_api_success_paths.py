...
class TestLambdaEndpointSuccessPaths:
    ...
    def test_lambda_success_minimal_config(
        self, mock_lambda_calc, mock_validate, client
    ):
    ...
    @patch("app.api.calculator_api.validate_lambda_inputs")
    @patch("app.api.calculator_api.lambda_calc")
    def test_lambda_success_with_custom_egress_rates(
        self, mock_lambda_calc, mock_validate, client
    ):
        ...
        custom_rates_arg = (
            mock_lambda_calc.calculate_total_cost.call_args[0][1]
            if len(call_args) > 1
            else None
        )
    ...
    @patch("app.api.calculator_api.validate_lambda_inputs")
    @patch("app.api.calculator_api.lambda_calc")
    def test_lambda_success_without_custom_egress_rates(
        self, mock_lambda_calc, mock_validate, client
    ):
        ...
        mock_lambda_calc.calculate_total_cost.return_value = {"total_cost_usd": 2.35}
...
class TestComparisonEndpointSuccessPaths:
    ...
    def test_comparison_success(
        ...
        mock_lambda_calc.calculate_total_cost.return_value = {"total_cost_usd": 12.34}

        mock_vm_calc.calculate_vm_cost.return_value = {"monthly_cost_usd": 7.59}
...
class TestServerlessEndpointSuccessPaths:
    ...
    def test_serverless_gcp_functions_success(
        self, mock_serverless_calc, mock_validate, client
    ):
...
class TestCSVExportEndpointSuccessPaths:
    ...
    def test_export_csv_success(
        self, mock_create_csv, mock_vm_calc, mock_lambda_calc, mock_validate, client
    ):
        ...
        mock_create_csv.return_value = (
            "provider,instance_type,lambda_total_cost_usd\naws_ec2,t3.micro,13.49\n"
        )
