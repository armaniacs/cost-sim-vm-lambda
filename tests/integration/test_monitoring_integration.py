    ...
    def test_logging_integration(self, app):
        ...
        with app.app_context():
            ...
            if logging_service:
                ...
                recent_logs = logging_service.aggregator.get_recent_logs(limit=10)
                assert len(recent_logs) > 0
                
                # Check log structure
                log_entry = recent_logs[-1]
    ...
    def test_alerting_integration(self, app):
        ...
        with app.app_context():
            ...
            if alerting_service:
                ...
                rule_names = [rule.name for rule in alerting_service.alert_rules.values()]
    ...
    def test_observability_health_checks(self, app):
        ...
        with app.app_context():
            ...
            if observability_service:
                ...
                health_summary = observability_service.health_checker.get_health_summary()
...
class TestMonitoringSecurityAndCompliance:
    ...
    def test_sensitive_data_handling(self, client):
        ...
        if response.status_code == 200:
            ...
            for pattern in sensitive_patterns:
                assert pattern not in logs_content.lower(), f"Sensitive data found: {pattern}"
    
    def test_audit_logging(self, client):
        """Test audit logging functionality."""
        # Trigger an audit event
        response = client.post("/api/v1/monitoring/events", json={
            "type": "audit",
            "event": "user_login",
    ...
    def test_access_control(self, client):
        ...
        assert response.status_code in [404, 405, 500]  # Should not exist or be protected
