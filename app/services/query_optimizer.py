...
class IndexRecommender:
    ...
    def analyze_table_queries(
        self, table_name: str, queries: List[str]
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_join_columns(self, query: str, table_name: str) -> List[str]:
        ...
        join_pattern = (
            r"JOIN\s+\w+\s+ON\s+(.*?)(?=\s+(?:WHERE|ORDER|GROUP|HAVING|LIMIT|JOIN|$))"
        )
...
class QueryOptimizer:
            ...
            if issue["type"] == "select_star":
                ...
                if "users" in optimized.lower():
                    optimized = optimized.replace(
                        "SELECT *", "SELECT id, email, created_at, is_active"
                    )
                elif "calculations" in optimized.lower():
                    optimized = optimized.replace(
                        "SELECT *", "SELECT id, configuration, result, created_at"
                    )
                elif "pricing" in optimized.lower():
                    optimized = optimized.replace(
                        "SELECT *",
                        "SELECT id, provider, region, service, price, timestamp",
                    )
                else:
                    optimized = (
                        f"-- Optimized: Replace SELECT * with specific columns\n"
                        f"{optimized}"
                    )
                ...
                if "users" in optimized.lower():
                    optimized = optimized.replace(
                        "FROM users", "FROM users WHERE is_active = true"
                    )
                elif "calculations" in optimized.lower():
                    optimized = optimized.replace(
                        "FROM calculations",
                        (
                            "FROM calculations WHERE created_at >= "
                    ...
                    optimized = optimized.replace(
                        "FROM pricing",
                        "FROM pricing WHERE timestamp >= NOW() - INTERVAL '7 days'",
                    )
                else:
                    optimized = (
                        f"-- Optimized: Add WHERE clause to limit results\n{optimized}"
                    )
    ...
    def get_table_optimization_report(self, table_name: str) -> Dict[str, Any]:
        ...
        recommendations = self.index_recommender.analyze_table_queries(
    ...
    def _get_table_statistics(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                return {"column_statistics": stats, "analysis_timestamp": time.time()}
