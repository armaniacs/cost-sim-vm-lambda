...
class LogAggregator:
    ...
    def _matches_query(self, log: Dict[str, Any], query: Dict[str, Any]) -> bool:
        ...
        if (
            "message" in query
            and query["message"].lower() not in log["message"].lower()
        ):
            return False
...
class LogAnalyzer:
    ...
    def analyze_error_patterns(self) -> Dict[str, Any]:
        ...
        with self.aggregator.lock:
            error_logs = [
                log
                for log in self.aggregator.logs
    ...
    def analyze_performance_logs(self) -> Dict[str, Any]:
        ...
        with self.aggregator.lock:
            perf_logs = [
                log for log in self.aggregator.logs if log["level"] == "PERFORMANCE"
            ]
        ...
        return {"total_performance_logs": len(perf_logs), "insights": insights}
...
class EnhancedLogHandler(logging.Handler):
    ...
    def _store_in_redis(self, log_entry: Dict[str, Any]):
        ...
        try:
            ...
            # Log Redis connection issues but don't affect main application
            logging.getLogger(__name__).debug(f"Redis logging failed: {e}")
...
class LoggingService:
    ...
    def _add_file_handlers(self, logger: logging.Logger, log_dir: Path):
        ...
        for level_name, level in handlers:
            ...
            handler.addFilter(
                lambda record, target_level=level: record.levelno >= target_level
            )
    ...
    def log_audit_event(self, event: str, details: Dict[str, Any], user_id: str = None):
        ...
        audit_logger.log(
            AUDIT_LEVEL,
            event,
            extra={"event_type": "audit", "details": details, "user_id": user_id},
        )

    def log_security_event(
        self, event: str, details: Dict[str, Any], severity: str = "medium"
    ):
        ...
        security_logger.log(
            SECURITY_LEVEL,
            event,
            extra={"event_type": "security", "details": details, "severity": severity},
        )
    ...
    def log_performance_event(self, event: str, metrics: Dict[str, Any]):
        ...
        performance_logger.log(
            PERFORMANCE_LEVEL,
            event,
            extra={"event_type": "performance", "metrics": metrics},
        )
...
def log_api_call(
    include_request_data: bool = False, include_response_data: bool = False
):
