...
class AlertingService:
    ...
    def get_active_alerts(
        ...
        if tags:
            alerts = [
                alert
                for alert in alerts
                if all(alert.tags.get(k) == v for k, v in tags.items())
            ]
    ...
    def suppress_alert(
        self, alert_id: str, duration: timedelta, reason: str, user: str = None
    ) -> bool:
        ...
        with self.lock:
            if alert_id in self.active_alerts:
                ...
                alert.suppress(until, user or "system", reason)
                self.escalation_manager.stop_escalation(alert_id)
                logger.info(
                    f"Alert {alert_id} suppressed by {user or 'system'} for "
                    f"{duration.total_seconds() / 60} minutes: {reason}"
                )
                return True
            return False
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        """Get dashboard data with alerts summary."""
        with self.lock:
        ...
        acknowledged_count = sum(
            1 for alert in active_alerts if alert.status == AlertStatus.ACKNOWLEDGED
        )
        suppressed_count = sum(1 for alert in active_alerts if alert.is_suppressed())
        ...
        recent_alerts = sorted(alert_history, key=lambda a: a.created_at, reverse=True)[
            :10
        ]
    ...
    def search_alerts(self, query: Dict[str, Any]) -> List[Alert]:
    ...
    def _matches_alert_query(self, alert: Alert, query: Dict[str, Any]) -> bool:
        """Check if an alert matches the search query."""
        # Check severity
        if "severity" in query:
            if alert.severity.value != query["severity"]:
                return False

        # Check status
        if "status" in query:
            if alert.status.value != query["status"]:
                return False

        # Check rule name
        if "rule_name" in query:
            if alert.rule_name != query["rule_name"]:
                return False

        # Check metric name
        if "metric_name" in query:
            if alert.metric_name != query["metric_name"]:
                return False

        # Check tags
        if "tags" in query:
            query_tags = query["tags"]
            if not all(alert.tags.get(k) == v for k, v in query_tags.items()):
                return False

        return True

    def _create_alert(
        self, rule: AlertRule, value: float, timestamp: datetime, tags: Dict[str, str]
    ) -> Optional[Alert]:
        ...
        alert = Alert(
            ...
            message=(
                f"{rule.name}: {rule.metric_name} = {value} {rule.condition} "
                f"{rule.threshold}"
            ),
    ...
    def _process_alert(self, alert: Alert):
        ...
        # Correlation
        self.correlation_engine.correlate_alert(alert)
            ...
            try:
                ...
                if handler:
                    ...
                    if success:
                        ...
                        logger.debug(
                            f"Notification sent via {channel.value} for "
                            f"alert {alert.id}"
                        )
                    else:
                        logger.warning(
                            f"Failed to send notification via {channel.value} for "
                            f"alert {alert.id}"
                        )
                else:
                    logger.warning(
                        f"No handler found for notification channel: {channel.value}"
                    )
                ...
                "levels": [
                    ...
                    },
                    {
                        ...
                        "actions": [
                            ...
                            {
                                "type": "webhook",
                                "url": "https://hooks.company.com/escalation",
                            },
                        ],
                    },
                ]
    ...
    def _start_background_workers(self):
        ...
        escalation_worker = threading.Thread(
            target=self._escalation_worker, daemon=True
        )
    ...
    def _cleanup_alert_history(self):
        ...
        if len(self.alert_history) >= self.alert_history.maxlen:
            logger.debug(
                "Alert history at capacity, oldest entries will be "
        ...
        with self.lock:
            ...
            for alert in self.active_alerts.values():
                ...
                active_alerts_by_severity[severity] = (
                    active_alerts_by_severity.get(severity, 0) + 1
                )
