                ...
                "attachments": [
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "title": "Value",
                                "value": f"{alert.value} {alert.condition} {alert.threshold}",
                                "short": True
                            },
                            {
                                "title": "Time",
...
class EscalationManager:
    ...
    def start_escalation(self, alert: Alert, policy_id: str):
        ...
        with self.lock:
            ...
            escalation_data = {
                ...
                'next_escalation_at': datetime.utcnow() + timedelta(minutes=policy['levels'][0]['delay_minutes'])
            }
    ...
    def process_escalation(self, escalation_data: Dict[str, Any]):
        ...
        if escalation_data['current_level'] < len(policy['levels']):
            ...
            escalation_data['next_escalation_at'] = datetime.utcnow() + timedelta(minutes=next_level['delay_minutes'])
...
class AlertingService:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        with self.lock:
            active_alerts_list = [alert.to_dict() for alert in self.active_alerts.values()]
            recent_alerts = [alert.to_dict() for alert in list(self.alert_history)[-50:]]
    ...
    def search_alerts(self, query: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        with self.lock:
            alerts_to_search = list(self.active_alerts.values()) + list(self.alert_history)
