...
class MonitoringIntegration:
    ...
    def _initialize_services(self):
        """Initialize all monitoring services."""
        try:
            # Skip APM service initialization during testing to avoid
            # request context issues
            is_testing = self.app.config.get("TESTING", False)

            # Initialize APM service
            if self.config.get("apm.enabled", True) and not is_testing:
    ...
    def _setup_service_integrations(self):
        ...
        if "apm" in self.services and "alerting" in self.services:
            ...
            if "observability" in self.services:
                ...
                observability_service.add_custom_collector(
                    "apm_metrics", lambda: apm_service.get_dashboard_data()
                )
        ...
        if "performance" in self.services and "observability" in self.services:
            ...
            observability_service.add_custom_collector(
                "performance_metrics", lambda: performance_monitor.get_dashboard_data()
            )
    ...
    def _register_endpoints(self):
        ...
        monitoring_bp = Blueprint(
            "monitoring", __name__, url_prefix="/api/v1/monitoring"
        )
        ...
        def health_check():
            ...
            try:
                if "observability" in self.services:
                    health_data = self.services[
                        "observability"
                    ].health_checker.get_health_summary()
                    return jsonify(health_data)
                else:
                    return jsonify(
                        {"status": "healthy", "message": "Monitoring services running"}
                    )
            except Exception as e:
        ...
        def get_metrics():
            ...
            try:
                ...
                if "observability" in self.services:
                    ...
                    return (
                        jsonify({"error": "Observability service not available"}),
                        404,
                    )
            except Exception as e:
                return jsonify({"error": str(e)}), 500
        ...
        def acknowledge_alert(alert_id):
            ...
            try:
                if "alerting" in self.services:
                    ...
                    success = self.services["alerting"].acknowledge_alert(
                        alert_id, user_id
                    )
                    if success:
    ...
    def get_unified_dashboard(self) -> Dict[str, Any]:
        ...
        dashboard = {"timestamp": datetime.utcnow().isoformat(), "services": {}}
    ...
    def _generate_summary_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "performance" in self.services:
                ...
                summary["average_response_time"] = perf_data["performance"][
...
def get_monitoring_service(service_name: str) -> Any:
    """Get a specific monitoring service."""
...
def log_audit_event(
    event: str, details: Dict[str, Any], user_id: Optional[str] = None
) -> None:
    """Log an audit event."""
