...
class Span:
    ...
    def __init__(self, span_id: str, operation_name: str, parent_span_id: Optional[str] = None):
    ...
    def get_all_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                'histograms': {k: self._analyze_histogram(v) for k, v in self.histograms.items()},
...
class APMService:
    ...
    def after_request(self, response):
        ...
        if hasattr(g, 'trace_context') and hasattr(g, 'current_span'):
            ...
            self.metrics.record_timer('request.duration', duration, {
    ...
    def teardown_handler(self, exception):
        ...
        if exception and hasattr(g, 'current_span'):
            ...
            self.error_tracker.record_error(exception, {
                'trace_id': g.trace_context.trace_id if hasattr(g, 'trace_context') else None,
    ...
    def handle_error(self, error):
        ...
        self.error_tracker.record_error(error, {
            'trace_id': g.trace_context.trace_id if hasattr(g, 'trace_context') else None,
            'span_id': g.current_span.span_id if hasattr(g, 'current_span') else None,
            'endpoint': request.endpoint,
            'method': request.method
        })
...
def record_custom_metric(metric_name: str, value: Union[int, float], 
