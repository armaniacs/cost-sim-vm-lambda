...
Unified calculator for AWS Lambda, Google Cloud Functions/Cloud Run, Azure Functions, and OCI Functions
...
class ServerlessConfig:
    ...
    provider: str  # "aws_lambda", "gcp_functions", "gcp_cloudrun", "azure_functions", "oci_functions"
    memory_mb: int
...
class ServerlessResult:
    ...
    breakdown: Dict[str, float]  # {"request_cost": 0.1, "compute_cost": 1.5, "memory_cost": 0.3}
...
class AWSLambdaProvider(ServerlessProvider):
    ...
    def calculate_cost(self, config: ServerlessConfig) -> ServerlessResult:
        ...
        if config.include_free_tier:
            billable_requests = max(0, config.monthly_executions - self.FREE_TIER_REQUESTS)
        else:
        ...
        # Compute charges
        gb_seconds = (config.memory_mb / 1024) * config.execution_time_seconds * config.monthly_executions
        ...
        # Free tier savings
        free_tier_request_savings = min(config.monthly_executions, self.FREE_TIER_REQUESTS) / 1_000_000 * self.REQUEST_PRICE_PER_MILLION if config.include_free_tier else 0
        free_tier_compute_savings = min(gb_seconds, self.FREE_TIER_GB_SECONDS) * self.COMPUTE_PRICE_PER_GB_SECOND if config.include_free_tier else 0
        ...
        return ServerlessResult(
            ...
            breakdown={
                ...
                "egress_cost": 0.0  # Will be calculated separately by egress calculator
            },
            free_tier_savings={
            ...
            resource_usage={
                ...
                "requests_free": self.FREE_TIER_REQUESTS if config.include_free_tier else 0,
                "gb_seconds_used": gb_seconds,
                "gb_seconds_free": self.FREE_TIER_GB_SECONDS if config.include_free_tier else 0
            },
            features={
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        return {
            ...
            "free_tier": f"{self.FREE_TIER_REQUESTS:,} requests + {self.FREE_TIER_GB_SECONDS:,} GB-seconds/month",
...
class GCPFunctionsProvider(ServerlessProvider):
    ...
    def calculate_cost(self, config: ServerlessConfig, include_egress_free_tier: bool = None) -> ServerlessResult:
        ...
        # Free tier savings
        free_tier_request_savings = min(config.monthly_executions, self.FREE_TIER_REQUESTS) * self.REQUEST_PRICE if config.include_free_tier else 0
        free_tier_compute_savings = min(gb_seconds, self.FREE_TIER_GB_SECONDS) * self.COMPUTE_PRICE_PER_GB_SECOND if config.include_free_tier else 0
    ...
    def validate_config(self, config: ServerlessConfig) -> List[str]:
        ...
        if not (128 <= config.memory_mb <= 8192):
            errors.append("Memory must be between 128 and 8,192 MB for Google Cloud Functions")
        
        if not (0.1 <= config.execution_time_seconds <= 540):
            errors.append("Execution time must be between 0.1 and 540 seconds for Google Cloud Functions")
    ...
    def calculate_egress_charges(self, config: ServerlessConfig, include_egress_free_tier: bool = None) -> float:
        """
        ...
        use_egress_free_tier = include_egress_free_tier if include_egress_free_tier is not None else config.include_free_tier
...
class ServerlessCalculator:
    ...
    def __init__(self):
        ...
        logger.info(f"ServerlessCalculator initialized with providers: {list(self.providers.keys())}")
    
    def calculate(self, config: ServerlessConfig, include_egress_free_tier: bool = None) -> ServerlessResult:
        """
        ...
        if config.provider not in self.providers:
            ...
            raise ValueError(f"Provider '{config.provider}' not supported. Available: {available_providers}")
        ...
        # Calculate cost
        try:
            if hasattr(provider, 'calculate_cost') and 'include_egress_free_tier' in provider.calculate_cost.__code__.co_varnames:
                result = provider.calculate_cost(config, include_egress_free_tier)
            else:
                result = provider.calculate_cost(config)
            logger.info(f"Calculated cost for {config.provider}: ${result.total_cost_usd:.4f}")
            return result
    ...
    def calculate_multiple(self, configs: List[ServerlessConfig]) -> List[ServerlessResult]:
        """
        ...
        for config in configs:
            try:
                ...
                logger.error(f"Failed to calculate cost for {config.provider}: {str(e)}")
