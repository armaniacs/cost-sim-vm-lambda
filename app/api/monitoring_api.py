...
def health_check():
    ...
    try:
        ...
        if not monitoring_integration:
            return (
                jsonify({"status": "error", "message": "Monitoring not initialized"}),
                500,
            )
...
def get_metrics():
    ...
    try:
        ...
        if monitoring_integration.is_service_enabled("observability"):
            observability_service = monitoring_integration.get_service("observability")
            metrics = observability_service.get_metrics_for_dashboard(
...
def get_dependencies():
    ...
    try:
        ...
        if monitoring_integration.is_service_enabled("observability"):
            observability_service = monitoring_integration.get_service("observability")
            dependencies = (
                observability_service.dependency_mapper.get_dependency_graph()
            )
...
def update_config():
    ...
    try:
        ...
        return jsonify(
            {"status": "updated", "timestamp": datetime.utcnow().isoformat()}
        )
