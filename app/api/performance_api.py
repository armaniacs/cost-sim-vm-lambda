...
def invalidate_cache():
    ...
    try:
        ...
        return jsonify({
            'success': success,
            'message': f'Cache pattern {pattern} invalidated' if success else 'Invalidation failed',
            'timestamp': datetime.utcnow().isoformat()
        })
...
def get_index_recommendations():
    ...
    try:
        ...
        if table_name:
            ...
            tables = ['pricing_snapshots', 'calculation_history', 'cost_optimization_recommendations']
            recommendations = {}
...
def get_endpoint_metrics():
    ...
    try:
        ...
        for endpoint, times in performance_monitor.metrics.endpoint_times.items():
            endpoint_metrics[endpoint] = performance_monitor.get_endpoint_metrics(endpoint)
    ...
    try:
        ...
        report = {
            ...
            'summary': {
                ...
                'average_response_time': performance_monitor.metrics.get_average_response_time(),
                'p95_response_time': performance_monitor.metrics.get_95th_percentile_response_time(),
        ...
        if format_type == 'json':
            ...
            # Write CSV headers
            writer.writerow([
                'Metric', 'Value', 'Threshold', 'Status', 'Timestamp'
            ])
            
            # Write performance data
            for metric, value in metrics.items():
                status = 'OK' if value < 80 else 'WARNING' if value < 95 else 'CRITICAL'
                threshold = 80 if metric == 'cpu_usage' else 85 if metric == 'memory_usage' else 90
                writer.writerow([
            ...
            if db_metrics:
                ...
                for metric, value in db_metrics.items():
                    status = 'OK' if value < 100 else 'WARNING' if value < 500 else 'CRITICAL'
                    writer.writerow([
                        metric.replace('_', ' ').title(),
            ...
            response.headers['Content-Disposition'] = f'attachment; filename=performance_report_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}.csv'
...
def get_performance_config():
    ...
    try:
        config = {
            ...
            'cache_config': {
                'redis_enabled': cache_service.cache.redis_cache.redis_client is not None,
...
def update_performance_config():
    ...
    try:
        ...
        if 'alert_thresholds' in data:
            performance_monitor.alert_manager.alert_thresholds.update(data['alert_thresholds'])
